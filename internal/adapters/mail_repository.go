...
func NewSmtpMailRepo(cfg *config.MailConfig) (*mailRepo, error) {
	return &mailRepo{cfg: cfg}, nil
}

// Send sends a mail.
func (r *mailRepo) Send(_ context.Context, subject, body string, to []string, options *model.MailOptions) error {
	if options == nil {
		options = &model.MailOptions{}
	}
...
func (r *mailRepo) setDefaultOptions(sender string, options *model.MailOptions) {
	if options.ReplyTo == "" {
